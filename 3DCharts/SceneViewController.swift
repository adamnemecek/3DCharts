...
class SceneViewController : UIViewController, SCNSceneRendererDelegate {
    ...
    private func showData() {
        ...
        chartNode.rotation = SCNVector4(x: 1, y: 0, z: 0, w: -Float.pi/2)
    }
    ...
    private func createChart(_ type:ChartType) -> SCNNode {
        ...
        for idx in 0 ..< numColumns {
            ...
            let posY = min.y - 2.0
            txtNode.position = SCNVector3(x: posX, y: posY, z: 0.0)
            txtNode.rotation = SCNVector4(x: 0, y: 0, z: 1, w: -Float.pi/2)
            node.addChildNode(txtNode)
        }
        ...
        for i in 0 ..< numRows {
            for j in 0 ..< numColumns {
                ...
                aNode.rotation = SCNVector4(x: 1, y: 0, z: 0, w: -Float.pi/2)
    ...
    private func createPieChart() -> SCNNode {
        ...
        for j in 0 ..< numColumns {
            ...
            startRad = Float(startDeg) * .pi / Float(180.0)
            let endDeg = startDeg + pct - 1.0
            let endRad:Float = Float(endDeg) * .pi / Float(180.0)
            let circlePath = UIBezierPath()
            circlePath.move(to: .zero)
            circlePath.addArc(withCenter: .zero,
                              radius: 20.0,
    ...
    private func turnCameraAroundNode(_ node:SCNNode, radius:Float)
    {
        ...
        for index in 0 ..< 360 {
            let hAngle = Double(index) * .pi / 180.0
            let vAngle = Double(-45) * 0.25 * .pi / 180.0
